...
class DiscriminatorNet(torch.nn.Module):
    def __init__(self,channels,factor, Kernel_factor,z_dim,num_condition,
                 num_pixels, Minibatch,PacGAN_pacnum, mode_collapse, gen_num):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(
                in_channels= (1 + self.num_condition) * self.Pack_num, out_channels=self.channels//8, kernel_size=3,
        ...
        if Minibatch:
            ...
            self.linear1 = nn.Sequential(
                nn.Linear(self.channels * self.init_pixel * self.init_pixel, self.n_out),
            )
    ...
    def forward(self, x, y):
        ...
        x = x.reshape(int(x.shape[0]),1 * self.Pack_num, self.pixels_edge, self.pixels_edge)
...
#
def initialize_models(channels,factor, Kernel_factor,z_dim, num_condition, num_pixels,Minibatch,minibatch_kind,
                      Pack_number,PacGAN_pacnum, mode_collapse, gen_num):
    init_pixel = 2
    if Minibatch:
        minibatch_net = MinibatchDiscrimination(channels * init_pixel * init_pixel,
                                                channels * init_pixel * init_pixel, 5,minibatch_kind)
    else:
        minibatch_net = False

    discriminator = DiscriminatorNet(channels,factor, Kernel_factor,z_dim, num_condition,
                                     num_pixels,minibatch_net, PacGAN_pacnum, mode_collapse, gen_num)
    generator = GeneratorNet(channels,factor, Kernel_factor,z_dim, num_condition, num_pixels)
    varnet = VARNet(channels,factor, Kernel_factor,z_dim, num_condition,num_pixels,minibatch_net, Pack_number)
    ...
    # print(generator, discriminator, varnet, g_optimizer, d_optimizer, var_optimizer, loss_BCE)

    return generator, discriminator, varnet, g_optimizer, d_optimizer, var_optimizer, loss_BCE
