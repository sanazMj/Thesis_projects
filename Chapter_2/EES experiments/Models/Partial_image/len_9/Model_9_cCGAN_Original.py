...
class DiscriminatorNet(torch.nn.Module):
    ...
    def __init__(self,channels,z_dim,num_condition,num_pixels, Minibatch, PacGAN_pacnum):
        ...
        if Minibatch:
            self.linear1 = nn.Sequential(
                nn.Linear(channels // 1 * self.final_pixel * self.final_pixel, 1),
                nn.Sigmoid()
            )
        else:
            self.linear1 = nn.Sequential(
                nn.Linear(channels//2* self.final_pixel*self.final_pixel ,1),
                nn.Sigmoid()
            )
    ...
    def forward(self, x, y):
        ...
        if Print:
            print('d',x.shape,y.shape, self.packnum)
        x = x.reshape(int(x.shape[0]), 1*self.packnum, self.pixels_edge, self.pixels_edge)
        # x = x.reshape(int(x.shape[0]/self.packnum),1*self.packnum,self.pixels_edge,self.pixels_edge)
        if Print:
...
class VARNet(torch.nn.Module):
    ...
    def forward(self, x, y):
        # Convolutional layers
        if Print:
            print('v',x.shape,y.shape, self.packnum)
        x = x.reshape(int(x.shape[0]), 1*self.packnum, self.pixels_edge, self.pixels_edge)
        # x = x.reshape(int(x.shape[0]/self.packnum),1*self.packnum,self.pixels_edge,self.pixels_edge)
        if Print:
...
#
def initialize_models(channels,channel_facotr, kernel_factor, z_dim, num_condition, num_pixels,Minibatch, minibatch_kind, packnum,PacGAN_pacnum):
    ...
    discriminator = DiscriminatorNet(channels,z_dim, num_condition,num_pixels,minibatch_net,PacGAN_pacnum)
