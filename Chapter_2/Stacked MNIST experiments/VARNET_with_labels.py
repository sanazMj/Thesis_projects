...
def my_config():
    ...
    # num_training_sample = 10000
    # num_test_sample = 2600
    num_runs = 10
    n_features = 100
...
def main(Model, Model_structure, Model_type, model_kind, mode_collapse, img_size, num_epochs, batch_size, num_training_sample, num_test_sample,
    ...
    for epoch in range(num_epochs):
        print('epoch', epoch)
        print('We should be here after the break')
        D_losses = []
        G_losses = []
        V_losses = []
        # learning rate decay
        ...
        for x_, y_ in train_loader_Stacked:
            ...
            # print(x_.shape, x_reshaped.shape, y_real_.shape, discriminator(x_reshaped).shape)
            ...
            if same==False:
                # print(x_.shape)
                fake_data_same, y_fake_reshaped =  create_sim_data_labeled(x_, y_, type_same)
                print('fake data, lable', fake_data_same.shape, y_fake_reshaped.shape)
                ...
                print('fake data, lable',fake_data_same_reshape.shape, y_fake_reshaped.shape)
            else:
            ...
            varnet.zero_grad()
            # print(x_reshaped.shape, varnet(x_reshaped).shape)
            v_result = varnet(x_reshaped).reshape(1)
            # print(v_result.shape,y_real_reshaped.shape, v_result, y_real_reshaped)
            v_real_loss = loss(v_result, y_real_reshaped)

            v_result = varnet(repeated_data).reshape(1)
            ...
            D_result = discriminator(G_result_reshaped_dim).squeeze()
            G_train_loss = loss(D_result, y_real_)

            v_result = varnet(G_result_reshaped).reshape(1)
