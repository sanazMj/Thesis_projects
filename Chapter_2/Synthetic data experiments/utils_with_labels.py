...
def evaluate_modes_reverse_KL(generator, test_samples, q, q_without_thresh, n_mixtures, tensor_loc, label,
                              std_dev=0.01):
    ...
    KL_thresh, KL_Score_thresh, KL_non_thresh, KL_Score_non_thresh = JSD().KLD(p, q_without_thresh), JSD().JSD_core(p,
                                                                                                                    q_without_thresh), JSD().KLD(
        p1, q_without_thresh), JSD().JSD_core(p1, q_without_thresh)
...
#
#     return error_real + error_fake, prediction_real, prediction_fake
def train_varnet(optimizer, varnet, criterion, real_data_reshape, total_label_diff, real_data, num_var, pac_var,
                 batch_size, n_mixture, Label_images, random_sample, same_creation_type):
    optimizer.zero_grad()
    ...
    if same_creation_type == 0 or same_creation_type == 1:
        fake_data_same, label = create_sim_data(real_data, batch_size, pac_var, n_mixture, Label_images, random_sample,type=same_creation_type)
        prediction_fake = varnet(fake_data_same)
        error_fake += criterion(prediction_fake, label)

    else:
        fake_data_same, label = create_sim_data_new(real_data, batch_size, pac_var, n_mixture, Label_images, random_sample,type=same_creation_type)
...
def train_generator_varnet(optimizer, discriminator, varnet, criterion, var_coef, pac_var, fake_data, fake_data1,
                           label1, same_creation_type=0):
    n = fake_data.size(0)
...
def create_sim_data(imgs, batch_size, pac_var, n_mixture, Label_images, random_sample,type=0):
    '''
...
def create_sim_data_new(imgs, batch_size, pac_var, n_mixture, Label_images, random_sample, type=0):
    '''
    Construct the similar images for VARNET training
    :param imgs: generator output
    :return: The first element of generator ouput is repeated and passed
...
def create_dif_data(real_data, pac_var, batch_size, type=0):
    if type == 0 or type == 1:
        ...
        for index, pac_var_element in enumerate(pac_var):
            ...
            real_data_reshape = real_data_temp.reshape(real_data_temp.shape[0] // pac_var_element,
                                                       real_data_temp.shape[1] * pac_var_element).cuda()
